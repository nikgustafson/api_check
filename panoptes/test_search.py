...
def test_FacetSearch(configInfo, facetName, facetID, facetPath, facetValue):
    ...
    if adminProductList['Meta']['TotalCount'] == 0:
        ...
        # log.debug('creating '+str(len(buyerProductIDs)) + ' Products with XP
        # '+facet['ID']+'!')
    ...
    '''
    for item in newBuyerProducts['Meta']['Facets']:
       # log.debug(newBuyerProducts['Meta']['Facets'][item])
...
def test_EX1663(configInfo):
    ...
    # 4. see the new Facets subobject on Meta, but there are no values
    # populated
    ...
    log.info('Total Initial Facets: ' +
             str(facetList.json()['Meta']['TotalCount']))
    ...
    if facetList.json()['Meta']['TotalCount'] == 0:
        for item in newFacets:
            try:
                log.info(item)
                facet = admin.post(
                    configInfo['API'] + 'v1/ProductFacets', json=newFacets[item])
        ...
        assert facetList.status_code is codes.ok
        log.info('Total Facets After Creation: ' +
                 str(facetList.json()['Meta']['TotalCount']))

    # update ten random products to have these facets

    pageSize = '10'
    productList = admin.get(
        configInfo['API'] + 'v1/Products', params={'pageSize': pageSize}).json()
    ...
    selectedProducts = admin.get(configInfo[
                                 'API'] + 'v1/Products', params={'page': randomPage, 'pageSize': pageSize})
    ...
    patchBody = {'xp': {}}
    sizeList = ['xs/0-4', 's/5-8', 'm/9-12',
                'l/13-16', 'xl/17-22', 'xxl/23-28']
    ...
    for facet in newFacets:
        ...
        if newFacets[facet]['XpPath'] == 'sizes':
            ...
            patchBody['xp'][newFacets[facet][
                'XpPath']] = random.choice(sizeList)

        else:
            patchBody['xp'][newFacets[facet]['XpPath']] = bool(
...
def test_NoFacetsReturnedForNonFacetLists(configInfo, buyer_endpoint):
    ...
    should_not_have_facet_meta = buyer.get(
        configInfo['API'] + 'v1/Me/' + buyer_endpoint)
