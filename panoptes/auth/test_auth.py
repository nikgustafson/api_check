...
def test_updatePriceSchedules(configInfo):
    ...
    priceSchedules = session.get(
        configInfo['API'] + 'v1/priceSchedules', params={'PageSize': '100'})
    ...
    if totalCount <= 100:
        for item in priceSchedules.json()['Items']:
            log.info(item['ID'])
            old = session.get(configInfo['API'] +
                              'v1/priceSchedules/' + item['ID'])
            new = session.patch(
                configInfo['API'] + 'v1/priceSchedules/' + item['ID'], json=newBody)
...
def test_anonOrderNotFound(configInfo):
    ...
    lineitem = anon.post(
        configInfo['API'] + 'v1/orders/outgoing/' + orderID + '/lineitems', json=newLine)
